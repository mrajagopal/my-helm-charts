...
for ((i = 0; i < ${COUNT}; i++)) {
	...
	helm install ${HELM_INSTALL_NAME} oci://ghcr.io/nginx/charts/nginx-ingress --version ${CHART_VERSION}  -f ./values-wafv4.yaml --namespace ${NAMESPACE} --create-namespace --set controller.ingressClass.name="${INGRESS_CLASS_NAME}" --set controller.image.tag=${TAG}
